...
import { toDynamo, fromDynamo, parseNumberOrThrow } from '../src/index';
...
const allGroups = async () => {
    ...
    return [
        ...
        groupTest('Js To Dynamo', test('a1.Number To Dynamo', toDynamo(1), { N: '1' }), test('b1.String To Dynamo', toDynamo('1'), { S: '1' }), test('c1.Buffer To Dynamo', toDynamo(Buffer.from('Buf1')), { B: Buffer.from('Buf1') }), test('d1.String Set To Dynamo', toDynamo(['', '', '']), { SS: ['', '', ''] }), test('e1.Buffer Set To Dynamo', toDynamo([Buffer.from('Buf1'), Buffer.from('Buf1')]), { BS: [Buffer.from('Buf1'), Buffer.from('Buf1')] }), test('e2.Buffer Set To Dynamo', toDynamo([Buffer.from('Buf1'), Buffer.from('Buf1')]), { BS: [Buffer.from('Buf1'), Buffer.from('Buf1')] }), test('f1.Number Set To Dynamo', toDynamo([1, 2, 3]), { NS: ['1', '2', '3'] }), test('f2.Number Set To Dynamo', toDynamo([1, 2, 3]), { NS: ['1', '2', '3'] }), test('g1.List To Dynamo', toDynamo([1, true, '3', Buffer.from('Buf1'), null]), { L: [{ N: '1' }, { BOOL: true }, { S: '3' }, { B: Buffer.from('Buf1') }, { NULL: true }] }), test('g2.List To Dynamo', toDynamo([1, true, '3', Buffer.from('Buf1'), null]), { L: [{ N: '1' }, { BOOL: true }, { S: '3' }, { B: Buffer.from('Buf1') }, { NULL: true }] }), test('1. toDynamo is equiv to built in converter', toDynamo(testToDyanmo[0].input), convertToDynamo(testToDyanmo[0].input)), test('2. toDynamo is equiv to built in converter', toDynamo(testToDyanmo[1].input), convertToDynamo(testToDyanmo[1].input)), test('3. toDynamo is equiv to built in converter', toDynamo(testToDyanmo[2].input), convertToDynamo(testToDyanmo[2].input)), test('4. toDynamo throws an Error for odd inbound data types', isErrorThrown(() => toDynamo(new Error('someStrange input type'))), true), test('5. toDynamo throws an Error for odd inbound data types', isErrorThrown(() => toDynamo(new Date())), true)),
