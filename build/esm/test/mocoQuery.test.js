...
const allGroups = async () => {
    ...
    return groupTest('MocoQuery Query Builder', groupTest('Easy Tests', test('Check Defaults', mocoQuery('table1').extract(), {
        ...
        .select('ALL_PROJECTED_ATTRIBUTES')
        .select('BAD INPUT')
        .extract()), true), test('Expression Attr Values', () => mocoQuery('table1').expressionAttributeValues({ a: true }).extract(), () => { }), test('Dyanmo Query to URL', () => mocoQuery('table1')
        .select('COUNT')
        .where(['pk', '=', 'Value1'])
        ...
        .consistentRead(true)
        .toUrlString(), () => 'dynamo://table1/pk+=+%22Value1%22/sk+begins_with+%22prefix%22?attr1+begins_with+%22prefix%22&AND+attr2+=+null&AND+attr3Not42+%3C%3E+42#ConsistentRead=true&Select=%22COUNT%22&ReturnConsumedCapacity=%22TOTAL%22'), test('Dyanmo Query from URLstring', () => mocoQuery('').fromUrl(mocoQuery('table1')
        .select('COUNT')
        .where(['pk', '=', 'Value1'])
        .where(['AND', ['sk', 'begins_with', 'prefix']])
        .filter(['attr1', '<>', false])
        ...
        .descending()
        .consistentRead(true)
        .toUrlString()).toUrlString(), () => mocoQuery('table1')
        .select('COUNT')
        .where(['pk', '=', 'Value1'])
        .where(['AND', ['sk', 'begins_with', 'prefix']])
        .filter(['attr1', '<>', false])
        ...
        .filter(['AND', ['attr3Not42', '<>', 42]])
        .consistentRead(true)
        .toUrlString()), test('Out of Order Filters should give equiv URL objects', () => mocoQuery('table1')
        .select('COUNT')
        .where(['pk', '=', 'Value1'])
        .where(['AND', ['sk', 'begins_with', 'prefix']])
        .filter(['attr1', '<>', false])
        ...
        .filter(['AND', ['attr3Not42', '<>', 42]])
        .consistentRead(true)
        .toURL()), test('Basic URL', mocoQuery('table1').toURL(), new URL('dynamo://table1#Select=All')), test('dummy', 1, 1));
};
(async () => {
    if (module.parent) {
        return allGroups;
