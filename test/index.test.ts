...
const findTestPaths = async (...globs:string[]) => {
    if(globs.length > 0){
        ...
        return globby(['**/*test.ts','!test/index.test.ts' ])
    }
    ...
    return Promise.all(
        ...
        .map(p => {
            ...
            return import(fullPath)
                .then(importedTest=>importedTest.default)
            })
    )
}
...
;(async ()=>{
    if(!module.parent){
        const tests = (await importTests(findTestPaths())) as (()=>PromiseLike< ITestReturn | ITestReturn[] | IGroupReturn[] >)[]
