...
const base64toBuff = (s:string):Buffer => Buffer.from(s, 'base64')
export const reservedWords = (decompressSync: IbrotliDecompressSync) => {
  // const compressedBuff = fs.readFileSync(path.resolve(__dirname, './src/reserved.txt.br'))
  // inlined the code so that we dont need the asset from fs
  const reservedWordCompressedBuff = base64toBuff(
    'GyYQAIzUWg2v7UaodChVyfptO8aeWIvTmX6RCuBXf4rf7nUjNsnxvhGcrCRtD6fmf/e007lQYZ6xAlYAcyfJrqQMjyjDI0N2' +
...
interface BrotliOptions {
  // @default constants.BROTLI_OPERATION_PROCES
  flush?: number;
